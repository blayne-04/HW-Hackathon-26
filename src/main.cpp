// ============================================
// ⚠️ CRITICAL: Blynk defines MUST be at the VERY TOP
// ============================================
#define BLYNK_TEMPLATE_ID "YOUR_TEMPLATE_ID"      // Replace with your Template ID
#define BLYNK_TEMPLATE_NAME "Sound Classifier"    // Replace with your Template name
...
#define VIRTUAL_PIN_MUTE V2             // Mute control (0=Unmute, 1=Mute)
#define VIRTUAL_PIN_LED_TEXT V3         // Short LED text for display widget
#define VIRTUAL_PIN_ALERT_STATE V4      // Numeric alert state (0=none,1=smoke,2=doorbell)
#define VIRTUAL_PIN_STATUS V5           // System status

// ============================================
// DISPLAY CONFIGURATION (GC9A01 Round Display)
// ============================================
TFT_eSPI tft = TFT_eSPI();
const int NUM_BARS = 12;
const int BAR_WIDTH = 240 / NUM_BARS;   // 20 pixels per bar
float barMagnitudes[NUM_BARS] = {0};

// ============================================
// I2S MICROPHONE CONFIGURATION (INMP441)
// ============================================
#define I2S_WS 25      // Word Select (L/R)
#define I2S_SCK 26     // Bit Clock (SCK)
#define I2S_SD 33      // Data In (SD)
...
// ============================================
// LED STRIP CONFIGURATION (WS2811 12V)
// ============================================
...
// ============================================
// RELAY OUTPUTS (for external alerts)
// ============================================
...
const float DOORBELL_RATIO = 0.35;      // Doorbell: 800-1500 Hz
const float SMOKE_RATIO = 0.45;          // Smoke alarm: 2800-3500 Hz
...
const unsigned long COOLDOWN_MS = 8000;   // 8 seconds cooldown between alerts

// ============================================
// REMOTE MUTE (from Blynk)
// ============================================
...
void sendAlertToBlynk(AlertState state, String fullText)
{
    // Send full text to notification pin (V1)
...
// ============================================
// LED LOUDNESS BAR (Physical LEDs)
// ============================================
void showLoudnessBar()
{
    long sumSq = 0;
...
void setup()
{
    ...
    sendAlertToBlynk(ALERT_NONE, "NONE");
    Blynk.virtualWrite(VIRTUAL_PIN_MUTE, 0);  // Initialize mute to OFF
    
    Serial.println("Setup complete. Listening...");
    Serial.println("Virtual Pins: V1=Notification, V2=Mute, V3=LED Text, V4=Alert State, V5=Status");
}
...
void loop()
{
    ...
    updateAlert();    // Handle alert animations
    updateDisplayAlert();
    ...
    // Skip audio processing if alert is active
    if (currentAlert != ALERT_NONE)
    {
        delay(10);
        return;
    ...
    // Compute band energies for classification
    float totalEnergy = 0;
    ...
    for (int i = 1; i < samplesFFT / 2; i++)
    {
        ...
        totalEnergy += vReal[i];
        if (freq >= 800 && freq <= 1500)      // Doorbell frequency range
            energyDoorbell += vReal[i];
        if (freq >= 2800 && freq <= 3500)     // Smoke alarm frequency range
            energySmoke += vReal[i];
    }

    // Prepare spectrum bars for display (12 frequency bands)
    ...
    // Sound classification and alert triggering
    ...
    {
        // Audio detected but system is muted
